...
public class CommConnection {
	...
	private void init() {
		...
		try {
			...
			//New Code to add create an instance of CommInitializer to send it to the handler  
			CommInitializer ci = new CommInitializer(handler, false);
			//End Code to add create an instance of CommInitializer to send it to the handler 
			...
			while (true) {
				...
				try {
					...
					if (ch.isWritable()) {
						...
						if (!handler.send(msg , ch)) // Added channel to the send message
